...
const AddWeather = function ({ cdata }) {
  ...
  if (cdata && cdata.response && cdata.response.body && cdata.response.body.items && cdata.response.body.items.item) {
    ...
    for(i=0; i<oldr.length; i++) { //서울특별시 삼선동 성북구 ... -> 서울특별시 삼선동
      if(oldr[i] === " ") {
        r += (oldr[i]);
        n++;
        if(n >= 2) break;
  ...
  useEffect(() => {
    let url = `https://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/getVilageFcst?serviceKey=jSLG7PhndCZp9dBtSCY5UGFS4dLgXrtHWCe4JURn1K7VE7UDXwRv9xyHgez0UaGVP8L9%2Bv22bAKf9Uy%2BPWrFeQ%3D%3D&pageNo=1&numOfRows=1000&dataType=JSON&base_date=${""+year+month+date}&base_time=0500&nx=${rs.x}&ny=${rs.y}`;
